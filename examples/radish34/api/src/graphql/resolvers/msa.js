...
const getSignatureStatus = msa => {
  ...
  if (RBuyer && SBuyer)
    ...
    buyerSignatureStatus = false;
  if (RSupplier && SSupplier)
...
const mapMSAsWithSignatureStatus =
    async msas => {
  const msasWithSignatureStatus = await Promise.all(msas.map(async msa => {
    const {constants, ...msaData} = msa;
    const {buyerSignatureStatus, supplierSignatureStatus} =
        getSignatureStatus(msa);
    const supplier =
        await getPartnerByzkpPublicKey(constants.zkpPublicKeyOfSupplier);
    return {
      ...msaData,
      ...constants,
...
export default {
  ...
  Mutation : {
    createMSA : async (_parent, args, context) => {
      ...
      logger.info(`Sending MSA with id ${msaDoc._id} to ${
                      _supplier.messagingKey} for signing.`,
                  {service : 'API'});
  ...
  Subscription : {
    newMSA : {
      subscribe : () => { return pubsub.asyncIterator(NEW_MSA); },
    },
  },
};
