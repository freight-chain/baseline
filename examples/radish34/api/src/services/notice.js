...
export const getNoticesByCategory = async category => {
  const notices = await db.collection('notices').find({category}).toArray();
...
export const getInbox = async () => {
  const notices =
      await db.collection('notices').find({status : 'incoming'}).toArray();
  return notices;
};

export const getOutbox = async () => {
  const notices =
      await db.collection('notices').find({status : 'outgoing'}).toArray();
  return notices;
};
