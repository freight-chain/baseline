...
import {createPO, getAllPOs, getPOById, PO, savePO} from '../../services/po';
...
export default {
    ...
    po:
        async (_parent, args) => {
          ...
          const supplier = await getPartnerByzkpPublicKey(
              po.constants.zkpPublicKeyOfSupplier);

          return { ...po, whisperPublicKeyOfSupplier: supplier.identity, }
        },
    pos() { return getAllPOs();},
  },
  Mutation: {
    createPO:
        async (_parent, args, context) => {
          ...
          try {
            ...
            currentUser: {
              ...
              identity:
            '0x042dd4912150fe2ecdbb4bd84a2f44dce6bac3530f2f6d702db27686898a8cd1185e3988aae09508680f815ed3494d11de18026ecd641da2786490a6fa3b33bf18'
              name: 'Org1',
              role: 1
            }
            */
            ...
            logger.info(`Calculated a price of ${price} for this PO.`,
                        {service : 'API'});
            ...
            let {accumulatedVolumeOrdered} =
                oldMSAObject.commitments[0].variables;
            ...
            const newMSA = new MSA({
              ...
              variables :
                  {...oldMSA.object.variables, accumulatedVolumeOrdered},
            });
            ...
            const supplier =
                await getPartnerByzkpPublicKey(zkpPublicKeyOfSupplier);
            ...
            logger.info(`Sending PO with id ${poDoc._id} to supplier.`,
                        {service : 'API'});
            ...
            pubsub.publish(NEW_PO, {
              newPO :
                  {...poDoc, whisperPublicKeyOfSupplier : supplier.messagingKey}
            });

            return {
              ...poDoc,
              whisperPublicKeyOfSupplier : supplier.messagingKey
            };
          } catch (err) {
            pubsub.publish('INCOMING_TOASTR_NOTIFICATION', {
              onNotification : {
                success : false,
                message :
                    'Something went wrong during PO creation, please try again.',
                userAddress : currentUser.address,
              }
            });
  ...
  Subscription: {
    newPO: {
      subscribe: () => { return pubsub.asyncIterator(NEW_PO);},
    },
  },
};
