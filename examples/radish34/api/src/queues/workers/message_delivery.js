...
module.exports = async (job, done) => {
  const { data: doc } = job;
  logger.info(
    `BullJS delivering message with uuid ${doc.documentId} to ${doc.recipientId}.`,
    { service: "API" }
  );
  const message = (
    await messenger.createMessage(doc.senderId, doc.recipientId, doc.payload)
  ).data;
  job.progress(50);

  logger.info(`BullJS updating the origination messageId to ${message._id}.`, {
    service: "API",
  });
